...
**Asking the agent in chat** ("what do you see on the front gate camera?") uses the `get_and_analyze_camera_image` tool instead of this service. The camera name from chat is resolved to a real `camera.*` entity with spaces, capitalization, and diacritics normalized ("kamera obývák 2" finds `camera.kamera_obyvak_2`; non-Latin names such as Cyrillic match too). If two cameras' names collide after normalization the tool refuses to guess, and an unknown name returns the list of available cameras (capped at 25) so the agent can correct itself. Note that resolution and the available-cameras hint consider every `camera.*` entity in Home Assistant, including cameras not exposed to Assist — expose-aware filtering is a tracked TODO. See [Architecture — Tools](architecture.md#tools) for the full tool list.
...
**Duplicate-frame collapse on battery cameras.** ring-mqtt's snapshot handling is power-aware: in `Interval` mode wired cameras refresh their snapshot image every 30 s but battery cameras only every 600 s, and in `Auto`/`Motion` modes battery cameras are never refreshed at all (see the snapshot-modes note below). A 30-second capture window therefore cannot yield distinct frames on a battery camera — it would batch ~10 identical copies of one frame, each costing a VLM call. To prevent this, the perceptual-hash uniqueness gate is always active for frames captured by a loop the `event_select` path started, regardless of the global uniqueness option (the first frame of each window is always accepted by the gate, so deduplication never leaves a capture window without an analysis). The forced gate persists even when a lagging motion sensor takes over the loop mid-window — the underlying frames still refresh at 600 s at best (or not at all in `Auto`/`Motion` modes) — and is retired when the loop stops. Frames the gate skips are still registered for normal snapshot retention, so duplicates never accumulate on disk. Loops started by a motion sensor keep the opt-in behavior unless an `eventId` arrives while they run ��� evidence the camera publishes ring-mqtt snapshots — which engages the forced gate for the remainder of that loop.

**Stale-snapshot guard.** The snapshot image of a battery camera can stop updating — either because the interval snapshot silently stalls, or because the camera is in a snapshot mode that never refreshes battery devices (see the snapshot-modes note below) — after which MQTT serves the last retained frame indefinitely (observed in the field at 5 days old). Before every capture, the analyzer checks the snapshot camera's `timestamp` attribute (epoch seconds of the last published frame); if the frame is older than 30 minutes, the capture is skipped and counted as a snapshot failure (escalating from WARNING to ERROR on repeats, as for issue #464) instead of sending days-old imagery to the VLM. The failure is recorded once per staleness episode — not once per 3-second loop iteration ��� where a changed frozen timestamp counts as a new episode and an ongoing freeze is re-recorded hourly (so a multi-day freeze stays visible in the hourly metrics and the escalation can reach ERROR); the episode clears when a fresh timestamp appears. If the camera is in `Interval` mode and was refreshing before, restarting the ring-mqtt add-on usually restores it; if it is in `Auto` or `Motion` mode, a restart will not help — the mode simply never requests battery snapshots, and the fix is the automation or mode change described below. The guard only applies to cameras with a ring-mqtt `event_select` sibling (matched by naming, the motion→camera override map, or a `select.*_event_select` entity on the same HA device �� the same device-registry resolution the trigger path uses for renamed entities): other integrations may publish a `timestamp` attribute with different semantics, and those cameras — as well as any camera whose timestamp is not a plausible current epoch (non-numeric, millisecond-scale, far-future) — are never marked stale.

**Battery cameras and ring-mqtt snapshot modes.** Field testing ([#466](https://github.com/goruck/home-generative-agent/issues/466)) showed that ring-mqtt's `Auto` (default) and `Motion`/`Motion + Ding` snapshot modes never request snapshots from battery cameras — not slowly, but zero requests. In those modes every frame an `event_select` capture window sees is the retained frame from a previous event; the stale-snapshot guard (correctly) rejects it once it exceeds 30 minutes, and events yield no analyzed frames. The camera hardware is not the limit: explicit `Interval` mode refreshes reliably, and the on-demand `button.<name>_take_snapshot` entity returned a fresh frame in roughly 0.2–3 s on the field-tested battery devices (Battery Doorbell Plus, Spotlight Cam — verify on your own hardware, since Ring models differ in whether they can snapshot while a recording is in progress).

For battery Ring cameras, add a per-camera automation that presses the take-snapshot button whenever a new Ring event is published. Because the analyzer's capture window opens on the same `eventId` change, the fresh frame lands early in the window (field-measured 160 ms–3 s depending on model) and a subsequent 3-second capture picks it up. The leftover previous-event frame captured at window open is rejected by the guard when it is over 30 minutes old; when it is younger than that it is still analyzed alongside the fresh frame (or, if the two scenes are nearly identical, the duplicate gate may keep only the earlier one), so a window may describe the previous event's scene as well — a planned refinement (comparing the frame timestamp against the event that opened the window) is tracked in TODOS.md. Note the condition on the trigger: without it, the automation would also fire when the entity returns from `unavailable` carrying its retained `eventId` (HA restart, MQTT broker reconnect, ring-mqtt add-on restart) and wake every battery camera at once — the same retained-replay shape the analyzer itself filters out.
...
Two expected side effects worth knowing: on a quiet camera (more than 30 minutes between events) the guard's stale-frame rejection at window open logs one snapshot-failure WARNING per event — that is the guard working as designed, not a sign the automation failed; and the button press wakes the battery camera once per Ring event, which on a very busy street-facing camera adds up (Ring's own motion cooldowns bound the rate — add a throttle condition to the automation if you want fewer wake-ups).

Alternatively, set the camera's snapshot mode to `Interval` in ring-mqtt — the frame then refreshes every 600 s on battery (30 s wired), which bounds staleness below the guard's threshold at the cost of more frequent battery wake-ups.
...
The previous frame's text also serves as motion/direction context for the captioner — but only for motion, never for identity. The captioner is instructed to describe each person from the current image alone and to never introduce someone as "another" or "a different" person relative to the previous frame's text: a single image cannot show whether its person matches the previous description, and captions that invented that distinction were a field-verified source of phantom second people (they also falsely tripped the summary constraint's contrast-cue veto). Wording like "another person" remains legitimate when one image itself shows two or more people at once.
