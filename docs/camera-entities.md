...
**Battery cameras and ring-mqtt snapshot modes.** Field testing ([tsightler/ring-mqtt#1103](https://github.com/tsightler/ring-mqtt/issues/1103)) showed that ring-mqtt's `Auto` (default) and `Motion`/`Motion + Ding` snapshot modes never request snapshots from battery cameras — not slowly, but zero requests. In those modes every frame an `event_select` capture window sees is the retained frame from a previous event; the stale-snapshot guard (correctly) rejects it once it exceeds 30 minutes, and events yield no analyzed frames. The camera hardware is not the limit: explicit `Interval` mode refreshes reliably, and the on-demand `button.<name>_take_snapshot` entity returned a fresh frame in roughly 0.2–3 s on the field-tested battery devices (Battery Doorbell Plus, Spotlight Cam — verify on your own hardware, since Ring models differ in whether they can snapshot while a recording is in progress).
