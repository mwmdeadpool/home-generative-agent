...
def test_streaming_failure_content_truncates_long_reason() -> None:
    """Reason text longer than the cap is truncated with an ellipsis."""
    long_reason = "x" * (_STREAM_ERROR_REASON_MAX_CHARS + 50)
    result = _streaming_failure_content(HomeAssistantError(long_reason))
    assert len(result) < len("I'm sorry, I was unable to respond: ") + len(long_reason)
    assert result.endswith("…")


def test_streaming_failure_content_non_hass_error_shows_type_name() -> None:
    """Non-HomeAssistantError exceptions surface only the class name."""
    result = _streaming_failure_content(ValueError("secret internal detail"))
